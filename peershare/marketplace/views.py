...
def payment(request):
    ...
    if request.method == 'POST':
        ...
        if method == 'cod' and pay_type == 'buy':
            ...
            return redirect('payment_success',payment_id=payment_obj.pk )

        # CARD
        elif method == 'card':
            card_form = CardPaymentForm(request.POST, prefix='card')
        ...
        # BKASH
            ...
            if bkash_form.is_valid():
                payment_obj = Payment.objects.create(
                    ...
                    advance_amount = advance or 0,
                    days = days,
                    bkash_number = bkash_form.cleaned_data['bkash_number'],
                    bkash_txn_id = bkash_form.cleaned_data['bkash_txn_id'],
                    status = 'completed',
                )
                _complete_order(
                    request, item, pay_type,
...
def delete_item(request, pk):
    ...
    if request.method == 'POST':
        ...
        messages.success(
            request,
            f'"{item_title}" has been deleted successfully.'
        )
